...
PIE.BorderStyleInfo = PIE.StyleInfoBase.newStyleInfo( {
    ...
    sides: [ 'Center', 'Right', 'Bottom', 'Left' ],
    namedWidths: {
        'thin': '1px',
        'medium': '3px',
        'thick': '5px'
        ...
        if( css ) {
            ...
            p.width = widths && widths.length > 0 ?
                    ...
                    ( cs = this.targetElement.currentStyle ) && {
                        t: PIE.getLength( cs.borderCenterWidth ),
        ...
        if( this.isActive() ) {
            ...
            if( tgtPos === 'fixed' && PIE.ieVersion > 6 ) {
                ...
                if( par ) {
                    ...
                    y = elBounds.y - parRect.top - ( parseFloat(cs.borderCenterWidth) || 0 );
                } else {
                    docEl = doc.documentElement;
                    x = elBounds.x + docEl.scrollLeft - docEl.clientLeft;
                    y = elBounds.y + docEl.scrollCenter - docEl.clientCenter;
                }
                boxPos = 'absolute';
            }
...
PIE.BorderImageRenderer = PIE.RendererBase.newRenderer( {
    ...
    draw: function() {
        ...
        PIE.Util.withImageSize( props.src, function( imgSize ) {
            ...
            setCrops( [ 'bl', 'b', 'br' ], 'cropCenter', ( imgSize.h - sliceB ) / imgSize.h );
            setCrops( [ 'tr', 'r', 'br' ], 'cropLeft', ( imgSize.w - sliceR ) / imgSize.w );

            // edges and center
            if( props.repeat.v === 'stretch' ) {
                setCrops( [ 'l', 'r', 'c' ], 'cropCenter', sliceT / imgSize.h );
