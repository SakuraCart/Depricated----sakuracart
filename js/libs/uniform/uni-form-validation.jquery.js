...
// validating a phone number might be useless!
phoneNumber = /\d+ ?\w{0,9} ?\d+/;
...
* ADDED: Date in MM-DD-YYYY, DD-MM-YYYY, YYYY-MM-DD format
         *
         * @param jQuery field
         * @param string caption
         */
        validateDate : function(field, caption) {
            if(field.val().match('(1[0-9]|[1-9])/([1-3][0-9]|[1-9])/((19|20)[0-9][0-9]|[0-9][0-9])')) {
                return true;
} else if (field.val().match('(1[0-9]|[1-9])/([1-3][0-9]|[1-9])/((19|20)[0-9][0-9]|[0-9][0-9])'))  {
	return true;
} else if (field.val().match('([1-3][0-9]|[1-9])/(1[0-9]|[1-9])/((19|20)[0-9][0-9]|[0-9][0-9])'))  {
	return true;
} else if (field.val().match('((19|20)[0-9][0-9]|[0-9][0-9])-(1[0-9]|[1-9])-([1-3][0-9]|[1-9])'))  {
	return true;
            }
 ...
 * moved English default to it's own file.
 */
