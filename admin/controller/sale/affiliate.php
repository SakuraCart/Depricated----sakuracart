...
class ControllerSaleAffiliate extends Controller {
	...
	public function index() {
		$this->data = array_merge($this->data, $this->load->language('sale/affiliate'));
...
$this->document->addBreadcrumbs(
	$this->url->link('sale/affiliate', 'token=' . $this->session->data['token'] . $url, 'SSL'),
...
$this->document->addBreadcrumbs(
	...
	$this->language->get('text_separator')
		...
		if (isset($this->request->post['approved'])) {
			...
			$this->data['approved'] = $affiliate_info['approved'];
		} else {
		...
		$this->data['cancel'] = $this->url->link('sale/affiliate', 'token=' . $this->session->data['token'], 'SSL');
