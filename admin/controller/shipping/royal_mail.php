...
class ControllerShippingRoyalMail extends Controller {
	...
	public function index() {
		$this->data = array_merge($this->data, $this->load->language('shipping/royal_mail'));
...
$this->document->addBreadcrumbs(
	$this->url->link('shipping/royal_mail', 'token=' . $this->session->data['token'], 'SSL'),
	...
	$this->language->get('text_separator')
		...
		// 1st Class Recorded		if (isset($this->request->post['royal_mail_1st_class_recorded_rate'])) {
		...
		// 2nd Class Standard		if (isset($this->request->post['royal_mail_2nd_class_standard_rate'])) {
