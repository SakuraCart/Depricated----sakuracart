...
<section class="box_content rounded_corners_all border_green">
	<h2 class="box_content rounded_corners_all border_blue gradient_blue">
		<img src="<?php echo ICON; ?>32/author.png" alt="lock" /> <?php echo $heading_title; ?>
	</h2>
						...
						<tr>
							...
							<td class="texttop">
								<div class="scrollbox">
									...
									<div class="<?php echo $class; ?>">
										<?php if (in_array(0, $author_store)) { ?>
											<input type="checkbox" name="author_store[]" value="0" checked="checked" />
											<?php echo $text_default; ?>
										<?php } else { ?>
											<input type="checkbox" name="author_store[]" value="0" />
											<?php echo $text_default; ?>
										<?php } ?>
									</div>
									<?php foreach ($stores as $store) { ?>
										...
										<div class="<?php echo $class; ?>">
											<?php if (in_array($store['store_id'], $author_store)) { ?>
												<input type="checkbox" name="author_store[]" value="<?php echo $store['store_id']; ?>" checked="checked" />
												<?php echo $store['name']; ?>
											<?php } else { ?>
												<input type="checkbox" name="author_store[]" value="<?php echo $store['store_id']; ?>" />
												<?php echo $store['name']; ?>
												<?php } ?>
										</div>
									<?php } ?>
							...
							<td class="texttop">
								<div class="image"><img src="<?php echo $thumb; ?>" alt="" id="thumb" />
									...
									&nbsp;&nbsp;|&nbsp;&nbsp;
									<a onclick="$('#thumb').attr('src', '<?php echo $thumb; ?>'); $('#image').attr('value', '');"><?php echo $text_clear; ?></a>
								</div>
							</td>
						</tr>
				</tbody>
