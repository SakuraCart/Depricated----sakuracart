...
<section class="box_content rounded_corners_all border_green">
	<h2 class="box_content rounded_corners_all border_blue gradient_blue">
		<img src="<?php echo ICON; ?>32/download.png" alt="lock" /> <?php echo $heading_title; ?>
	</h2>
				...
				<table id="datatable" class="datatable">
					...
					<tbody>
						<?php foreach ($downloads as $download) { ?>
							<tr>
								<td scope="row">
									<?php if ($download['selected']) { ?>
										<input type="checkbox" name="selected[]" value="<?php echo $download['download_id']; ?>" checked="checked" />
									<?php } else { ?>
										<input type="checkbox" name="selected[]" value="<?php echo $category['download_id']; ?>" />
									<?php } ?>
								</td>
