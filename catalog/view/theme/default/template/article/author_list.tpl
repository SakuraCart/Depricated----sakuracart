...
<div id="content">
	...
	<div class="box_content">
		<?php foreach ($categories as $category) { ?>
			<a href="index.php?route=article/author#<?php echo $category['name']; ?>"><?php echo $category['name']; ?></a>
		<?php } ?>
	</div>
	...
	<div class="box_plain">
		<?php foreach ($categories as $category) { ?>
			...
			<ul>
				<?php for ($j = 0; $j < count($category['author']);) { ?>
					<li>
						<div itemscope itemtype="http://schema.org/Article">
							<span itemprop="author">
								<a href="<?php echo $category['author'][$j]['href']; ?>"><?php echo $category['author'][$j]['name']; ?></a>
							</span>
						</div>
					</li>
					<?php $j++; ?>
