...
// ----------------------------------------------------------------------------------------------------
		$module_data = $this->cache->get('content_left.' . $layout_id . '.' . $this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id'));
		if (!$module_data) {
// ----------------------------------------------------------------------------------------------------
		$module_data = array();
...
// I used 0 since that is the first number. I could have used 'override' but that would defeat the purpose of localizations
					if (($module['layout_id'] == $layout_id && $module['position'] == 'content_left' && $module['status']) || ($module['layout_id'] == '0' && $module['position'] == 'content_left' && $module['status'])) {
						$module_data[] = array(
							'code'       => $extension['code'],
							'setting'    => $module,
							'sort_order' => $module['sort_order']
...
// ----------------------------------------------------------------------------------------------------
			$this->cache->set('content_left', $module_data);
		}
// ----------------------------------------------------------------------------------------------------
$this->data['column'] = '1';
