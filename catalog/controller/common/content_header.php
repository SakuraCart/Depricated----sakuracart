...
// ----------------------------------------------------------------------------------------------------
		$module_data = $this->cache->get('content_header.' . $layout_id . '.' . $this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id'));
		if (!$module_data) {
// ----------------------------------------------------------------------------------------------------
		$module_data = array();
...
// I used 0 since that is the first number. I could have used 'override' but that would defeat the purpose of localizations
					if (($module['layout_id'] == $layout_id && $module['position'] == 'content_header' && $module['status']) || ($module['layout_id'] == '0' && $module['position'] == 'content_header' && $module['status'])) {
						$module_data[] = array(
							'code'       => $extension['code'],
							'setting'    => $module,
...
// ----------------------------------------------------------------------------------------------------
			$this->cache->set('content_header', $module_data);
		}
// ----------------------------------------------------------------------------------------------------
